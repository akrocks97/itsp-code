...
void turn_right(float read_start){
    float read_final = read_start;
    if(abs(read_start)<90 || read_start < (-90) ){
      while(abs(read_final-read_start)<=90){
       ...
       //delay(1);//turn right for 1 ms
       read_final=reading();
     }  
    }
    else if(read_start > 90){
      float read_final;
      ...
      while(true){
        ...
        else if((angle1 + abs(180 - abs(read_final)))<90){
          analogWrite(enable_r, 190);
          analogWrite(enable_l, 190);
          digitalWrite(motor_r1,HIGH);
          digitalWrite(motor_r2,LOW);
...
void turn_left(float read_start){
    /*float read_final = read_start;
    if(abs(read_start)<90 || read_start >90){
      while(abs(read_final - read_start)<90){
       analogWrite(enable_r, 190);
      ...
      float read_final;
      read_final = read_start;
      float angle1 = abs(180- abs(read_start));
      while(true){
        if(read_final < -90){
          analogWrite(enable_r, 190);
          analogWrite(enable_l, 190);
...
void loop() {
  ...
  if (!dmpReady) return; //dmp not ready
  if(runno == 1){
    delay(17000);
    runno++;
