...
void turn_right(float read_start){
    ...
    if(read_start <90 ){ //read_start <90
...
void turn_left(float read_start){
    float read_final = read_start;
    if(read_start >(-90.0)){
      while(abs(read_start - read_final)<90.0){
      ...
      while(true){
       ...
       analogWrite(enable_l, 180);
       digitalWrite(motor_r1,LOW);
       digitalWrite(motor_r2,HIGH);
       digitalWrite(motor_l1,LOW);
       digitalWrite(motor_l2,HIGH);
        }
        else if((angle1 + abs(180.0- abs(read_final)))<90.0){
          Serial.println(read_final);
       analogWrite(enable_r, 255);
       analogWrite(enable_l, 255);
       digitalWrite(motor_r1,LOW);
